...
import static monorail.linkpay.jwt.DefaultJwtClaim.SALT;
...
public class JwtProvider {
     ...
     * @생각[주빈] 로그인 뿐만 아니라, 결제용 토큰 등 다른 분야에도 JWT가 사용될 것으로 예상되어 공통 모듈로 사용할 수 있도록 구성했습니다. 토큰 ID(만료 식별용/추적용)와 타입(용도 식별용)은 항상
     * 공통적으로 필요하다고 생각해서 메서드 시그니처에 포함시켰습니다. 나머지 정보는 Map의 형태로 호출하는 쪽에서 자유롭게 구성할 수 있는 게 더 낫다고 생각했습니다. 범용성은 높으나 호출하는 쪽에서 편하게
     * 쓰기 위해서는 별도로 어댑터 역할을 해줄 클래스를 추가로 구현해야 될 것 같습니다.
     */
