...
describe('ChargeModal', () => {
  it('모달이 렌더링되는지 확인', () => {
    render(<ChargeModal onClose={jest.fn()} onConfirm={jest.fn()} returnPage="mywallet" />);
  ...
  it('확인 버튼을 클릭하면 onConfirm이 호출되는지 확인', () => {
    ...
    render(<ChargeModal onClose={jest.fn()} onConfirm={mockConfirm} returnPage="mywallet" />);
    const input = screen.getByPlaceholderText('금액을 입력해주세요.');
    fireEvent.change(input, { target: { value: '5000' } });
  ...
  it('취소 버튼을 클릭하면 onClose가 호출되는지 확인', () => {
    ...
    render(<ChargeModal onClose={mockClose} onConfirm={jest.fn()} returnPage="mywallet" />);
